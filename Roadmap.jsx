...
const roadmapData = [
  {
    ...
    topics: [
      ...
      },
      {
        ...
        subtopics: [
          "JIT compilation: Ignition (interpreter) → TurboFan (optimizing compiler)",
        ...
        theory: "V8 compiles JS to machine code in two stages. Ignition generates bytecode fast; TurboFan optimizes hot functions. Hidden classes are internal type descriptors — adding properties in different orders creates different hidden classes, killing inline cache hits. For GC, young generation uses semi-space copying (Scavenger), old generation uses Mark-Sweep-Compact.",
      },
      {
      ...
      },
      {
        ...
        theory: "AsyncLocalStorage (async_hooks) lets you propagate context through async call chains without passing it manually — essential for request tracing and logging. AbortController (from Web API, now in Node) lets you cancel fetch, timers, and custom async work. Async iterators turn any data source into a consumable async stream.",
      },
      {
  ...
  },
  {
    ...
    topics: [
      ...
      },
      {
        ...
        theory: "Microservices aren't inherently better — they trade code complexity for operational complexity. Bounded contexts from DDD help you draw service boundaries correctly. Use synchronous communication for queries, asynchronous (RabbitMQ, Kafka) for commands. The Saga pattern replaces distributed transactions with a sequence of local transactions + compensating actions.",
      },
      {
      ...
      },
      {
        ...
        theory: "JWTs are stateless but can't be revoked easily — use short-lived access tokens with refresh token rotation. OAuth 2.0 delegates authentication; PKCE prevents authorization code interception. For security, think in layers: input validation (Joi/Zod), parameterized queries, CSP headers, rate limiting, and always hash passwords with bcrypt/argon2.",
      },
      {
        ...
        theory: "Horizontal scaling (more machines) beats vertical (bigger machine) for resilience. Cache at every layer: application (LRU), distributed (Redis), edge (CDN). Stateless services are key — externalize sessions to Redis so any instance can handle any request. Consistent hashing distributes load while minimizing redistribution when nodes join/leave.",
      },
    ],
  },
  {
    ...
    topics: [
      {
        ...
        theory: "PostgreSQL is the go-to for relational workloads. EXPLAIN ANALYZE shows actual execution plans — look for sequential scans on large tables (add indexes). B-tree for equality/range, GIN for full-text/JSONB, BRIN for naturally ordered data. Connection pooling is critical in Node because each connection is expensive (~10MB RAM). Use PgBouncer in transaction mode for microservices.",
      },
      {
        ...
        theory: "MongoDB's strength is flexible schema and horizontal scaling via sharding. Embed data that's read together; reference data that changes independently or grows unboundedly. The aggregation pipeline is MongoDB's SQL — learn it deeply. Change streams let you react to data changes in real-time (great for syncing caches, triggering workflows).",
      },
      {
        ...
        theory: "Redis isn't just a cache — it's a data structure server. Sorted Sets power leaderboards and priority queues. Redis Streams is a log-based structure similar to Kafka (with consumer groups). Redlock implements distributed locks across Redis instances. Lua scripts execute atomically on the server — use for read-modify-write operations that need atomicity.",
      },
      {
      ...
      },
      {
        ...
        theory: "Normalize first, denormalize for performance. Soft deletes (deleted_at column) keep data recoverable but complicate queries — use database views to abstract. Multi-tenancy choice depends on scale: row-level is simplest, schema-per-tenant balances isolation and cost, DB-per-tenant gives maximum isolation. Polyglot persistence means using Postgres for transactions, Redis for caching, Elasticsearch for search — each DB for its strength.",
      },
    ],
  },
  {
    ...
    topics: [
      {
        ...
        theory: "Multi-stage builds separate build dependencies from runtime — your final image should only contain production code and node_modules. Copy package.json before source code so npm install is cached when only code changes. Always run as non-root user. Handle SIGTERM for graceful shutdown: close server, finish in-flight requests, close DB connections, then exit.",
      },
      {
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ maxWidth: 800, margin: "0 auto" }}>
          ...
          <p style={{ fontSize: 14, opacity: 0.7, margin: "0 0 20px 0" }}>
            ~20–27 weeks • Theory-first approach • Intermediate → Senior
          </p>
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "24px 16px 60px" }}>
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}>
                    ...
                    <span style={{ fontSize: 11, color: "#9ca3af" }}>• {phase.duration}</span>
                  </div>
        ...
        >
          <strong style={{ color: "#334155" }}>💡 How to use this roadmap:</strong> Expand
          each phase and topic to read the theory. Check off subtopics as you learn them.
          Ask me to deep-dive into any specific topic — I'll explain the theory in detail
          with code examples.
        </div>
      </div>
    </div>
